...
如果您最近使用[Experience Platform标记](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/tags/home)实施了CX Enterprise，则已经为[客户属性](../services/overview.md)和CX Enterprise [受众](../services/audiences/overview.md)进行了设置。 您还可以在[Admin Console](../administration/admin-console.md)中管理用户和产品。
...
## 实施[!UICONTROL CX Enterprise ID服务]

[!UICONTROL CX Enterprise ID服务]为跨应用程序集成提供了一个通用ID。 它提供了跨域访客标识功能，并为基于通过[!DNL Customer Attributes]上传的CRM数据进行跨设备/浏览器定位和个性化提供了一种途径。

启用CX Enterprise核心服务的最简单方法是，通过[!UICONTROL Experience Platform Launch]中的[CX Enterprise ID Service扩展](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html?lang=zh-Hans)，为Analytics和Adobe Target自动激活核心服务。
...
如果您没有使用[!UICONTROL Experience Platform标记]，请通过JavaScript部署(`VisitorAPI.js`)手动实施ID服务，如下所示：

| 任务 | 描述 |
| --- | --- |
| [为Analytics实施CX Enterprise ID服务](https://experienceleague.adobe.com/zh-hans/docs/analytics/implementation/id/overview) | Adobe 还建议设置其他[客户 ID](https://experienceleague.adobe.com/zh-hans/docs/id-service/using/reference/authenticated-state)。 这些ID与每个访客相关联，并可以启用CX Enterprise中当前和未来的功能。 |
...
如果您使用的是第一方Cookie，请参阅[CNAME和CX Enterprise ID服务](https://experienceleague.adobe.com/docs/id-service/using/reference/analytics-reference/cname.html?lang=zh-Hans)以了解有关数据收集CNAME和跨域跟踪的信息。
...
* 建议您在[!UICONTROL Adobe Target]标记中添加[Experience Platform扩展](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/target-v2/overview.html?lang=zh-Hans)，以便自动检索库。 您还可以使用[!UICONTROL Enterprise ID]标记为Adobe Target（和其他应用程序）设置[CX Experience Platform服务扩展](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html?lang=zh-Hans)。 Adobe Target需要[!UICONTROL CX Enterprise ID服务]更新&#x200B;**&#x200B;**&#x200B;才能使用人员服务。
...
CX Enterprise [!UICONTROL 受众]是一个界面，您可以从这里创建受众，合并现有受众以创建组合受众，以及查看所有共享受众。
