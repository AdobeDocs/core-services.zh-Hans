...
如果您没有使用[!UICONTROL Experience Platform标记]，请通过JavaScript部署(`VisitorAPI.js`)手动实施访客ID服务，如下所示：
...
客户属性数据在访客登录后可用。 如果您尚未实施访客ID服务，并且以前一直在prop或eVar中跟踪客户ID，则可以请求一个流程，以将历史登录发送到CX Enterprise。 凭借此流程，您可以立即开始使用客户属性。
...
请使用以下流程以确保在您的网站上正确实施访客ID服务。

1. 清除网站的Cookie，以便您能够看到对访客ID服务的请求（请求在首次访问时发生，随后每位访客每周发生一次）。
1. 使用一个数据包分析程序或 Web 浏览器调试程序中的网络面板，查找到 [!DNL dpm.demdex.net] 的请求。
1. 验证响应中是否包含 `d_mid` 和一个值，例如：`_setMarketingCloudFields({"d_mid":"4235...`
1. 验证Analytics请求是否包含`mid`参数(ECID)。 在宽限期内（如果启用），您还应看到 `aid` 参数（Analytics 访客 ID）。

包含ECID的预期响应：

![包含ECID](../assets/mac_id_response.png)的预期响应

包含ECID（也称为`mid`或&#x200B;_访客ID_）的Analytics图像请求：

包含ECID的![Analytics图像请求](../assets/mid.png)

mbox请求中的ECID：

mbox请求中的![ECID](../assets/mbox_request.png)

### 什么是宽限期？

在您部署访客ID服务后，新访客将不再收到Analytics ID (`aid`)。 如果您网站的某些部分尚未实施访客ID服务，那么当访客浏览这些部分时，将无法识别ECID (`mid`)，与此同时，分配给访客的会是一个旧版Analytics ID (`aid`)。 此设置可能会导致潜在问题，包括重复访问和错误归因。

例如，如果网站的支持部分是在单独的 CMS 中管理的，则此部分可能有不同的 Analytics JavaScript 文件。 如果您在将访客ID服务部署到支持网站之前，在主要网站上部署了ECID，则新访客在访问支持部分时会收到一个旧版Analytics ID。 跨两个网站区域的访问报告为不同的访问。

在使用多个JavaScript文件或其他技术（例如Flash）的网站上部署访客ID服务时，可能会导致协调问题。 出现这些问题是因为您必须同时对网站的所有部分启用访客ID服务。 通过配置一个宽限期，新的访客可以继续从访客ID服务中接收Analytics访客ID。 对于网站中没有升级为使用访客ID服务的部分，可以始终如一地识别访客。
