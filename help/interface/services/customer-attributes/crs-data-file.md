---
description: 了解在 [!DNL Customer Attributes] 中将数据上载到CX Enterprise的数据文件要求和多个数据源。
...
将客户属性数据上传到CX Enterprise的数据文件要求和多个数据源。

您需要拥有从企业访问 CRM 或类似数据的权限。 上载到CX Enterprise的数据必须是`.csv`文件。 如果您通过 FTP 或 sFTP 上传，则还需要上传一个 `.fin` 文件。
...
<table id="table_A9849CC9AA784763921DE057F0F61515"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>您可以向CX Enterprise中的客户属性服务上传数百个<span class="filepath"> .csv </span>列。 但是，在配置订阅并选择属性时，根据您拥有的应用程序，将会受到以下限制： </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>上传客户属性数据时，如果您要快速且连续上传多个文件，尤其是文件较大时，请确保在上一个文件处理完成之后再上传下一个文件。 您可以通过查看上一个文件何时被移动到[!DNL Customer Attributes] FTP帐户中已处理或失败的文件夹来监控上传情况。 </p> <p> 如果将大文件分成较小的文件，并连续快速提交这些文件，实际上可能会减慢处理速度，除非您可以确保在提交下一个文件之前已完成处理每个文件。 </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p> 客户属性与[!DNL Analytics]中的基础访客配置文件关联。 因此，在[!DNL Analytics]中，[!DNL Customer Attributes]在访客配置文件的整个生命周期内都与该访客相关联。 该轮廓包括客户首次登录之前发生的行为。 </p> <p> 如果使用的是 Data Warehouse 回填方法，则数据将绑定到基于 Analytics ID (AID) 的 post_visid_high/low。 如果您使用访客ID服务，则数据将绑定到基于ECID (MID)的post_visid_high/low。 </p> <p> 请注意，从2022年10月开始，Data Warehouse回填方法将不再可用。 </td> 
  </tr> 
  <tr> 
...
**位于`VisitorAPI.js`或[!UICONTROL Experience Cloud ID服务]标记扩展中：**
