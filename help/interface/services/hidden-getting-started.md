...
如果您最近使用Experience Platform标记实施了CX Enterprise，则表明您已经为客户属性和CX Enterprise受众进行了设置。 您还可以在 Admin Console 中管理用户和产品。

现有客户可以使他们的应用程序实施实现现代化并实施CX Enterprise。 这样，您就可以在 Adobe Analytics、Audience Manager 和 Adobe Target 中使用客户属性和受众功能。

## 加入CX Enterprise并成为管理员 {#section_2423F0BD3DF642658103310EE5EA6154}

要加入CX Enterprise，您必须执行以下操作：
...
CX Enterprise菜单导航中提供&#x200B;**[!UICONTROL Admin Console]**&#x200B;链接。
...
查看CX Enterprise中的[关联帐户](../administration/organizations.md)
...
## 实施 [!UICONTROL CX Enterprise ID Service] {#section_3C9F6DF37C654D939625BB4D485E4354}
...
启用CX Enterprise核心服务的最简单方法是，通过[!UICONTROL Experience Platform Launch]中的[CX Enterprise ID服务扩展](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html?lang=zh-Hans)，为Analytics和Adobe Target自动激活核心服务。

有关完整的CX Enterprise ID服务帮助（以前称为访客ID），请转到[此处](https://experienceleague.adobe.com/docs/id-service/using/intro/overview.html?lang=zh-Hans#intro)。
...
| 任务 | 描述 |
| -----------| ---------- |
| [为Analytics实施CX Enterprise ID服务](https://experienceleague.adobe.com/docs/id-service/using/implementation/setup-analytics.html?lang=zh-Hans) | Adobe 还建议设置其他[客户 ID](https://experienceleague.adobe.com/docs/id-service/using/reference/authenticated-state.html?lang=zh-Hans)。 这些ID与每个访客相关联，并可以启用CX Enterprise中现有和未来的功能。 |
...
在设置CX Enterprise ID服务时，Adobe建议您针对Analytics和[!DNL Target]考虑将您的[客户ID](https://experienceleague.adobe.com/docs/id-service/using/reference/authenticated-state.html?lang=zh-Hans)与CX Enterprise同步。
...
当客户属性首次可用时，一些客户尚未实施CX Enterprise ID服务，并且无法轻松利用客户属性。 为了帮助缓解这个问题，Adobe 创建了一种使用 Adobe Analytics Data Warehouse 来回填 ID 同步的方法。 此功能称为 Data Warehouse 回填。 现在通常没有必要对 Data Warehouse 进行回填，因此从 2022 年 10 月起不再可用。


### Mobile SDK

有关如何在[™](https://experienceleague.adobe.com/docs/mobile-services/android/overview.html?lang=zh-Hans)和[Android](https://experienceleague.adobe.com/docs/mobile-services/ios/overview.html?lang=zh-Hans)移动设备应用程序中设置其他客户ID的语法示例，请参阅&#x200B;*iOS ID服务*&#x200B;部分。
...
## 将报表包映射到CX Enterprise组织 {#section_7B08516B01BA421681DF03D0E86CE3BA}

>[!NOTE]
>
>报表包映射功能已于 2020 年 11 月被弃用。 如有任何问题，请联系客户支持。

CX Enterprise服务（例如CX Enterprise ID服务）与CX Enterprise组织关联，而不是与单个Analytics报表包关联。 为确保这些服务正确运行，必须将每个Analytics报表包映射到CX Enterprise组织。

## 更新 Analytics AppMeasurement 代码 {#section_1798D9D0F05C47E29816AC4EEB9A0913}

如果您使用的是第一方Cookie，请参阅[CNAME和CX Enterprise ID服务](https://experienceleague.adobe.com/docs/id-service/using/reference/analytics-reference/cname.html?lang=zh-Hans)，了解有关数据收集CNAME和跨域跟踪的信息。
...
* 建议您在[!UICONTROL Experience Platform]标记中添加[Adobe Target扩展](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/target-v2/overview.html?lang=zh-Hans)，以便自动检索库。 您还可以使用[!UICONTROL Experience Platform]标记为Adobe Target（和其他应用程序）设置[CX Enterprise ID服务扩展](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html?lang=zh-Hans)。 需要[!UICONTROL CX Enterprise ID Service]更新&#x200B;**才能Adobe Target使用People服务**。
...
请使用以下流程以确保在您的网站上正确实施CX Enterprise ID服务。

1. 清除网站的Cookie，以便您能够看到对CX Enterprise ID服务的请求（请求在首次访问时发生，随后每位访客每周发生一次）。
...
![包含CX Enterprise ID](../assets/mac_id_response.png)的预期响应
...
在使用多个CX Enterprise文件或其他技术（例如Flash）的网站上部署JavaScript ID服务时，可能会导致协调问题。 出现这些问题是因为您必须同时对网站的所有部分都启用CX Enterprise ID服务。 通过配置一个宽限期，新的访客可以继续从ID服务中接收Analytics访客ID。 对于网站中没有升级为使用访客ID服务的部分，可以始终如一地识别访客。
