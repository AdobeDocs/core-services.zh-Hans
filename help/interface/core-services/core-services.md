...
seo-title: 为核心服务启用Experience Cloud解决方案
...
>[!NOTE]
>
>对于Target [，请从mbox. js迁移到. js](https://marketing.adobe.com/resources/help/en_US/target/ov2/t_target-migrate-atjs.html)。
...
有关帮助，请参阅 [Experience Cloud用户和产品管理](../admin-getting-started/admin-getting-started.md#topic_3FCB4099640647E3B2411ADBFCE81909) 。
...
> [!NOTE]
> 
> 映射企业和解决方案组后，将自动链接新用户。（将自动创建解决方案凭据并将这些凭据关联到其 Adobe ID。）

以下各部分描述了如何使您的实施符合现代化要求。在使您的实施符合现代化要求后，可以在 Experience Cloud 中启用核心服务。

## 步骤 2. 使用Dynamic Tag Manager或Launch实施Experience Cloud ID服务，由Adobe提供 {#section_3C9F6DF37C654D939625BB4D485E4354}

启用 Experience Cloud 核心服务的最简便方法是，通过动态标签管理器中的 [Experience Cloud ID 服务工具](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-dtm-implement.html)自动为 Analytics 和 Target 激活这些服务。（或者使用 Adobe 的 Launch。）
...
* 使用 `visitor.setCustomerIDs({"crm_id":"52mc210tr42"})` 访客ID服务调用。或，
...
有关 *如何在*[Android](https://marketing.adobe.com/resources/help/en_US/mobile/android/?f=methods) 和 [iOS](https://marketing.adobe.com/resources/help/en_US/mobile/ios/?f=methods) Mobile应用程序中设置其他客户ID的语法示例，请参见Experience Cloud ID服务部分。
...
建议您更新包括访客 API 在内的 JavaScript 库，以使您的 Analytics 实施现代化。完成这一操作的简单方法是在动态标签管理中添加 [!DNL Adobe Analytics] 一个工具，指定 *`Automatic`* 为配置方法。

在动态标签管理中，单击 **[!UICONTROL <Web Property Name>]**&gt;**[!UICONTROL “概述]**”&gt;“**[!UICONTROL 添加工具]**”&gt;“**[!UICONTROL Adobe Analytics]**”。请参阅动态标签管理中的[Adobe Analytics 设置](https://marketing.adobe.com/resources/help/en_US/dtm/?f=analytics_dtm)，以了解部署信息。

## 步骤 5. (Adobe Target) 使您的 Adobe Target 实施符合现代化要求 {#section_C2F4493C7A36406DAE2266B429A4BD24}

* 建议您在动态标签管理中添加 [Adobe Target 工具](https://marketing.adobe.com/resources/help/en_US/dtm/target.html)，以便自动检索您的库。在动态标签管理中，单击 **[!UICONTROL <Web Property Name>]**&gt;**[!UICONTROL “概述]**”&gt;**[!UICONTROL “添加工具]**”&gt;“**[!UICONTROL Adobe Target]**”。** 注意：**您还可以使用动态标签管理来为 Target（和其他解决方案）部署 Experience Cloud ID 服务。** 必须 **更新 Experience Cloud ID 服务，这样 Target 才能使用核心服务。
...
* 
   >[!IMPORTANT]
   >
   >所有分析客户已经为像客户属性这样的核心服务提供了供应。如果您不是 Analytics 客户，请联系客户关怀团队以请求进行配置。

## 步骤 6. 验证核心服务实施 {#section_E641782A0F4F44AF8C9C91216BE330D5}

使用下面的流程确保在您的网站上正确实施 Experience Cloud ID 服务。

1. 清除您网站上的 Cookie，这样可以看到对 Experience Cloud ID 服务的请求（该请求出现在第一次访问时，随后大约每周在每个访客访问时出现一次）。1. 在Web浏览器调试器中使用包分析器或网络面板查找请求 [!DNL dpm.demdex.net]。
1. 验证响应是否包含 `d_mid` 和值，例如： `_setMarketingCloudFields({"d_mid":"4235...`
...
## 第步：管理用户和产品 {#section_B6E95F4E0E12483CB9DA99CBC0C5A4AF}

启动并运行后，导航到 **[!UICONTROL “管理]** ”&gt; **[!UICONTROL “启动管理控制台]**”，您可以在其中管理用户和产品配置文件。

![](assets/menu-administration-shell.png)

请参阅 [Experience Cloud用户和产品管理](../admin-getting-started/admin-getting-started.md#topic_3FCB4099640647E3B2411ADBFCE81909)。
...
## 第步：开始使用核心服务 {#section_960C06093623462E8EA247B3E97274A1}
...
查看 [客户属性](../attributes/attributes.md#concept_ACFEE7C8B8E94875BA0825CDF4913AF1)
