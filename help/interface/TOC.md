---
role: User,Admin
user-guide-title: 《 CX Enterprise中央界面和管理指南》
breadcrumb-title: CX Enterprise界面和管理
title: 《 CX Enterprise界面和管理指南》
description: 了解CX Enterprise界面和管理。 浏览有关AI、管理、共享服务、数据收集、Cookie、发行说明和常见问题解答的主题。
index: true
source-git-commit: 25446910430bf15dcfa0fc70e25e0681f9faeb95
workflow-type: tm+mt
source-wordcount: '208'
ht-degree: 61%

---

# CX Enterprise界面和管理 {#interface}

+ [CX Enterprise界面和管理](/help/interface/experience-cloud.md)
+ CX Enterprise 中的 AI {#features}
   + [生成式 AI](/help/interface/features/generative-ai.md)
   + [代理式 AI](features/agentic-ai.md)
   + {hide-from-toc}[正在监视](/help/interface/features/monitoring.md)
   + [代理作业和AI信用消耗](/help/interface/features/ai-credit-consumption.md)
+ 管理 {#administration}
   + [帐户偏好设置和通知](/help/interface/features/account-preferences.md)
   + [浏览器和语言支持](/help/interface/browser-language.md)
   + [用户和产品管理](/help/interface/administration/admin-console.md)
   + [管理工具](/help/interface/administration/admin-tool-experience-cloud.md)
   + [组织和帐户关联](/help/interface/administration/organizations.md)
   + [集成](/help/interface/administration/integrations.md)
   + [统一搜索](/help/interface/features/search.md)
   + [个性化的学习偏好设置](/help/interface/features/personalized-learning.md)
+ 界面服务 {#services}
   + [界面概述](/help/interface/services/overview.md)
   + [快速入门](/help/interface/services/getting-started.md)
   + 资产 {#assets}
      + [Assets概述](/help/interface/services/assets/experience-cloud-assets.md)
      + [管理 Creative Cloud 用户](/help/interface/services/assets/manage-cc-users.md)
      + [共享资源文件夹](/help/interface/services/assets/share.md)
      + [将资源上传到共享文件夹](/help/interface/services/assets/upload.md)
      + [针对共享的资源文件要求](/help/interface/services/assets/file-reqs.md)
      + [共享文件夹的行为](/help/interface/services/assets/behavior.md)
   + 受众库 {#audiences}
      + [受众概述](/help/interface/services/audiences/overview.md)
      + [创建受众](/help/interface/services/audiences/create.md)
   + 客户属性 {#customer-attributes}
      + [客户属性概述](/help/interface/services/customer-attributes/attributes.md)
      + [常见问题](/help/interface/services/customer-attributes/faq-crs.md)
      + [数据文件和源](/help/interface/services/customer-attributes/crs-data-file.md)
      + [创建和上传数据](/help/interface/services/customer-attributes/t-crs-usecase.md)
      + [可选 - 通过 FTP 上传](/help/interface/services/customer-attributes/t-upload-attributes-ftp.md)
      + [验证架构](/help/interface/services/customer-attributes/validate-schema.md)
      + [配置订阅](/help/interface/services/customer-attributes/subscription.md)
      + [更新架构](/help/interface/services/customer-attributes/t-update-schema.md)
      + [支持 GDPR](/help/interface/services/customer-attributes/gdpr.md)
      + [支持 CCPA](/help/interface/services/customer-attributes/ccpa.md)
      + [隐私注意事项](/help/interface/services/customer-attributes/privacy-mac.md)
   + [触发器](/help/interface/services/triggers.md)
   + [Adobe Exchange](/help/interface/services/exchange.md)
+ 数据收集 {#data-collection}
   + [Adobe 管理的证书计划](/help/interface/data-collection/adobe-managed-cert.md)
   + [IP 地址](/help/interface/data-collection/ip-addresses.md)
   + [域](/help/interface/data-collection/domains.md)
   + Cookie {#cookies}
      + [Cookie 概述](/help/interface/data-collection/cookies/overview.md)
      + [Advertising Cookie](/help/interface/data-collection/cookies/advertising.md)
      + [Analytics Cookie](/help/interface/data-collection/cookies/analytics.md)
      + [Audience Manager Cookie](/help/interface/data-collection/cookies/audience-manager.md)
      + [CX企业版Cookie](/help/interface/data-collection/cookies/experience-cloud.md)
      + [Target Cookie](/help/interface/data-collection/cookies/target.md)
      + [Web SDK Cookie](/help/interface/data-collection/cookies/web-sdk.md)
   + [地区数据收集](/help/interface/data-collection/rdc.md)
   + [使用 DNS 预取](/help/interface/data-collection/dns-prefetch.md)
+ 发行说明和资源 {#more-resources}
   + [发行说明](/help/interface/more-resources/release-notes.md)
   + [文档更新](/help/interface/more-resources/doc-updates.md)
   + [常见问题](/help/interface/more-resources/faq.md)
   + [术语](/help/interface/more-resources/terms.md)
